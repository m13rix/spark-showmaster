...
function extractJSON(str) {
    ...
    if (startIndex === -1) {
        console.log('[extractJSON] No opening bracket found in:', str);
        return null;
    }
    ...
    try {
        ...
        console.log('[extractJSON] Successfully extracted JSON:', result);
        return result;
    } catch (e) {
        console.log('[extractJSON] Failed to parse JSON:', str.slice(startIndex, endIndex), 'Error:', e.message);
        return null;
    }
}
...
*   **Формат:** Весь код для одного события пишется в **ОДНУ СТРОКУ**, где переносы строк заменены на \`\\\\n\` (два обратных слеша, за которыми следует 'n'). Это необходимо для корректного экранирования символов новой строки в JSON, чтобы они оставались \`\\n\` после парсинга JSON и были правильно интерпретированы Nashorn как фактические переносы строк в JavaScript коде.
...
{
  "name": "НАЗВАНИЕ_ПРЕДМЕТА",
  "lore": "ОПИСАНИЕ ПРЕДМЕТА В ОДНОЙ СТРОКЕ!! НЕ МАССИВ, А ОДНА СТРОКА!!!",
  "base": "ТИП_ОСНОВЫ",
